[@felisdiligens/md-table-tools](README.md) / Exports

# @felisdiligens/md-table-tools

## Table of contents

### Enumerations

- [CSVTableRendererMode](enums/CSVTableRendererMode.md)
- [HTMLTableParserMode](enums/HTMLTableParserMode.md)
- [TableCaptionPosition](enums/TableCaptionPosition.md)
- [TableCellMerge](enums/TableCellMerge.md)
- [TextAlignment](enums/TextAlignment.md)

### Classes

- [CSVTableParser](classes/CSVTableParser.md)
- [CSVTableRenderer](classes/CSVTableRenderer.md)
- [GitHubFlavoredMarkdownTableParser](classes/GitHubFlavoredMarkdownTableParser.md)
- [GitHubFlavoredMarkdownTableRenderer](classes/GitHubFlavoredMarkdownTableRenderer.md)
- [HTMLTableParser](classes/HTMLTableParser.md)
- [HTMLTableRenderer](classes/HTMLTableRenderer.md)
- [MinifiedMultiMarkdownTableRenderer](classes/MinifiedMultiMarkdownTableRenderer.md)
- [MultiMarkdownTableParser](classes/MultiMarkdownTableParser.md)
- [PrettyMultiMarkdownTableRenderer](classes/PrettyMultiMarkdownTableRenderer.md)
- [Table](classes/Table.md)
- [TableCaption](classes/TableCaption.md)
- [TableCell](classes/TableCell.md)
- [TableColumn](classes/TableColumn.md)
- [TableRow](classes/TableRow.md)

### Interfaces

- [TableParser](interfaces/TableParser.md)
- [TableRenderer](interfaces/TableRenderer.md)
